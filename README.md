...
Daily aggregator of Nebraska-located taildragger classified listings,
...
[Montana](https://github.com/taildraggers/montana) repos' approach, with
the state filter swapped to "NE".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NE` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Nebraska"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/nebraska/`
...
```html
<iframe
  src="https://taildraggers.github.io/nebraska/"
  title="Taildragger Ads in Nebraska"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEBRASKA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
