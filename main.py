"""Aggregates Nebraska-located listings from every taildraggers.com
...
ones whose location ends in ", NE".
...
Minnesota/Mississippi/Missouri/Montana repos' approach, with the state
filter swapped to "NE".
"""
...
PAGE_TITLE = "Taildragger Ads in Nebraska"
STATE_ABBR = "NE"
...
_NEBRASKA_LOCATION_RE = re.compile(r",\s*NE\s*$", re.IGNORECASE)


def _is_nebraska(location: str) -> bool:
    return bool(_NEBRASKA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        nebraska_rows = [row for row in rows if _is_nebraska(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(nebraska_rows)} in Nebraska")
        all_listings.extend(nebraska_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Nebraska listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Nebraska listings: {len(listings)}")
